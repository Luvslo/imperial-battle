		...
		for ($i = 0; $i < count($shipdata); $i++) {
			...
			if (($ship_id == $ASTEROID_PIRATE) || ($ship_id == $NG_ASTEROID_PIRATE)) {
				...
				if ($ship_amount > 0) {
					...
					$target_steellost = $target_roidslost * ($target_steelroids / $target_totalroids);
					$target_crystallost = $target_roidslost * ($target_crystalroids / $target_totalroids);
					$target_erbiumlost = $target_roidslost * ($target_erbiumroids / $target_totalroids);
					$target_unusedlost = $target_roidslost * ($target_unusedroids / $target_totalroids);
					...
					for ($j = 0; $j < count($piratefleet); $j++) {
						...
						$steel_roids_capped = round($class_damage * ($target_steellost / $hitted_armor));
						$crystal_roids_capped = round($class_damage * ($target_crystallost / $hitted_armor));
						$erbium_roids_capped = round($class_damage * ($target_erbiumlost / $hitted_armor));
						$unused_roids_capped = round($class_damage * ($target_unusedlost / $hitted_armor));
