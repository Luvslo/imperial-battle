...
if ($do == 'alliancemgmnt') {
	if (!empty($_POST['createalliance'])) {
		...
		elseif ($num_checkalliance == 0) {
			$sql_createalliance = "INSERT INTO $table[alliance] (`name`, `tag`, `public_message`,`password`, `founder_id`, `startdate`, `message`, `message_lastedit`)
								VALUES ('$name', '$tag', 'This is the $name alliance.','$password', '$playerdata[id]', '$time', 'Welcome to the $name alliance.', '$time')";
	...
	if (!empty($_POST['joinalliance'])) {
		...
		if ($num_checkalliance > 0) {
			if (getAllianceMemberCount($rec_checkalliance['id']) >= $MAX_ALLIANCE_MEMBERS) {
				$error = 114;
			}
	...
	if (!empty($_POST['leavealliance'])) {
		if ($playerdata['alliance_id'] > 0) {
			$sql_la = "UPDATE $table[players] SET `alliance_id` = '0' WHERE `id` = '$playerdata[id]'";
			mysql_query($sql_la);
			if (mysql_affected_rows() == 1) { $error = 5; }
			else { $error = 112; }
		} else {
			$error = 113;
		}
...
switch($error) {
	...
	break;
	case 5:
	$msg = 'Succesfully left the alliance. You\'re on yourself again ;-)';
	break;
	case 100:
	...
	break;
	case 112:
	$msg = 'There went something wrong while leaving the alliance. Contact the game crew!';
	break;
	case 113:
	$msg = 'You are not in an alliance, so you can not leave one.';
	break;
	case 114:
	$msg = 'This alliance is full. There are only '.$MAX_ALLIANCE_MEMBERS.' members allowed for each alliance.';
}
                                        ...
                                        <td valign="top">
                                                <table border="0" width="800">
                                                        ...
                                                        <tr>
                                                                ...
                                                                <td width="650"><?if ($playerdata['alliance_id'] > 0) {echo getAllianceName($playerdata['id']).' - '.getAllianceTag($playerdata['id']);}?></td>
                                                        </tr>
                                                </table>
                                        </td>
                                </tr>
                        ...
                        <table border="0" cellpadding="0" cellspacing="0" width="800">
                                <tr>
                                        <td valign="top">
                                                ...
                                                <form method="POST" action="main.php?mod=other&act=myaccount&do=alliancemgmnt">
                                                <table border="0" width="800">
                                                        <tr>
                                                        	<td width="800" colspan="2" background="img/bg_balk.jpg"><b>Create new alliance</b></td>
                                                        </tr>
                                                        <tr>
                                                        ...
                                                        <tr>
                                                        	<td width="800" colspan="2" background="img/bg_balk.jpg"><b>Join alliance</b></td>
                                                        </tr>
                                                        <tr>
                                                        ...
                                                        <tr>
                                                        	<td width="800" colspan="2" background="img/bg_balk.jpg"><b>Leave current alliance</b></td>
                                                        </tr>
                                                        <tr>
                                                                <td width="800" colspan="2" align="center"><input type="submit" name="leavealliance" value="  Leave alliance  " onclick="return confirm('Are you sure you want to leave this alliance?')"></td>
                                                        </tr>
                                                        <?
                                                        }
                                                        ?>
