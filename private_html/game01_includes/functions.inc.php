...
function tryProductionAdd($player_id, $type_id, $item_id) {
	...
	if (checkItem($player_id, $item_id)) { return 1; }
	if (mysql_num_rows(mysql_query($sql_chkproduction2)) > 0) { return 2; }
	if (mysql_num_rows(mysql_query($sql_chkproduction1)) > 0) { return 3; }

	$res = mysql_fetch_array(mysql_query($sql_getitemdata));
	if (!checkItem($player_id, $res['depends'])) { return 4; }
...
function getProductionEta($player_id, $type_id, $item_id) {
	...
	if (@mysql_num_rows($res_productioneta) > 0) {
		...
		$eta = $rec_productioneta['ready_tick'] - getCurrentTick();
		return $eta;
	}
	return false;
}
...
function setCommander($galaxy_id) {
	...
	if ($highest_votes[2] >= $requiredvotes) {
		...
		$sql_newcommander = "UPDATE $table[galaxy] SET `commander_id` = '$highest_votes[0]' WHERE `id` = '$galaxy_id'";
...
function getARHtmlStart() {
	$html =
	'<table border="0" width="595" style="border: 1px solid #3C5762">
			<tr>
