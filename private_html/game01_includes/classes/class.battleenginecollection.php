...
class Fleet {
	...
	function updateShipsAmountByUniqueId($unique_id, $ships) {
				appendPlayerLog($ships->player_id, 'Updating amount by unique id '.$unique_id.'.', '');
		for ($i = 0; $i < count($this->shipCollection); $i++) {
			if ($this->shipCollection[$i]->getUnique() == $unique_id) {
				appendPlayerLog($ships->player_id, 'Found unique id in collection. Old amount: '.$this->shipCollection[$i]->getAmount().' - New amount: '.$ships->getAmount(), '');
...
class Ships {
	...
	function removeShip($percentage) {
		...
		appendPlayerLog($this->getPlayerId(), 'Removing ships (id '.$this->getId().') ('.$percentage.'%), amount before removal: '.$this->old_amount.'! Total armor: '.$total_armor.'. New armor: '.$new_armor.'. # of ships to remove: '.$rem_ships.'. ', '');
	}
