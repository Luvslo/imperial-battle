...
$sql_threaddata = "SELECT * FROM $table[allianceforum_threads] WHERE `id` = '$thread_id' AND `alliance_id` = '$playerdata[alliance_id]' ORDER BY `date` DESC";
...
if ($do == 'newreply') {
	...
	if ($error < 100) {
		...
		$sql_updatepostdate = "UPDATE $table[allianceforum_threads] SET `date` = '$date' WHERE `id` = '$thread_id'";
				...
				<tr>
					<td valign="top">
						<table border="0" width="800">
							...
							<?
							$sql_postdata = "SELECT * FROM $table[allianceforum_posts] WHERE `thread_id` = '$thread_id' AND `alliance_id` = '$playerdata[alliance_id]' ORDER BY `date`";
							...
							?>
							<tr>
								<td align="center">This thread has no posts, or is not a valid thread for your alliance.</td>
							</tr>
							<?
							}
							?>
